...
def high_confidence_solution(from_twitter, text, solution):
    ...
    text = text.upper()
    if from_twitter:
        reply = "😎 Sono quasi certo che la soluzione della #ghigliottina #leredita per (gli indizi {}) sia {}".format('{}',solution)
        text = restrict_text_to_twitter_limit(reply, text)
        return reply.format(text)        
    else:    
...
def good_confidence_solution(from_twitter, text, solution):
    ...
    text = text.upper()
    if from_twitter:
        reply = "🤔 Sono abbastanza convinto che la soluzione della #ghigliottina #leredita (per gli indizi {}) sia {}".format('{}',solution)
        text = restrict_text_to_twitter_limit(reply, text)
        return reply.format(text)
    else:    
...
def low_confidence_solution(from_twitter, text, solution):
    ...
    text = text.upper()
    if from_twitter:
        reply = "😜 Non sono sicuro, ma credo che la soluzione della #ghigliottina #leredita (per gli indizi {}) sia {}".format('{}',solution)
        text = restrict_text_to_twitter_limit(reply, text)
        return reply.format(text)
    else:    
